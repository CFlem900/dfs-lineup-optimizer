"""Compare main v5b lineups against DK simulator lineups for 03/05/2026 main slate."""
import csv
import json
import re
...
# Load our lineups
with open("main_v5b_lineups.json", encoding="utf-8") as f:
    ours = json.load(f)
...
# Load DK projections
dk_proj = {}
dk_proj_norm = {}  # normalized name -> original name
...
def get_dk_data(name):
    """Get DK projection data, handling name normalization."""
    if name in dk_proj:
        return dk_proj[name]
    nn = normalize(name)
...
def get_dk_exposure(name):
    """Get DK exposure count, handling name normalization."""
    if name in dk_exposure:
        return dk_exposure[name]
    nn = normalize(name)
    for dk_name in dk_exposure:
        if normalize(dk_name) == nn:
            return dk_exposure[dk_name]
    return 0


# All players (merge by normalized name)
all_norm = {}
for name in our_exposure:
    all_norm[normalize(name)] = name
for name in dk_exposure:
    nn = normalize(name)
...
for name in all_players:
    ...
    dk_exp_count = get_dk_exposure(name)
    dk_pct = (dk_exp_count / n_dk * 100) if dk_exp_count > 0 else 0
    gap = our_pct - dk_pct
    ...
    team = our_players[name]["team_abbreviation"] if name in our_players else dk_data.get("team", "?")
    salary = our_players[name]["salary"] if name in our_players else dk_data.get("salary", 0)
...
if dk_top20_missing:
    ...
    for n in dk_top20_missing:
        dd = get_dk_data(n)
        dk_exp = get_dk_exposure(n)
        print(f"  {n} ({dd.get('team','?')}) ${dd.get('salary',0):,} DK: {dk_exp/n_dk*100:.1f}%, FP={dd.get('fp',0):.1f}")
