...
data = json.load(open("v12_lineups.json", encoding="utf-8"))
print(f"=== v12 Full Results ===")
...
dk_sims_lineups = []
try:
    with open("C:/Users/CFlem/Downloads/DK_NBA_Main_Pre_Contest_Sims_Lineups (9).csv", encoding="utf-8") as f:
...
if n_dk > 0:
    ...
    close_count = len(dk_top20 & our_top20)

    print(f"\n--- Exposure vs DK Sims ---")
    print(f"Top-10 overlap: {overlap}/10")
    print(f"Top-20 overlap: {close_count}/20")

    print(f"\n{'Player':25s} {'Ours%':>6s} {'DK%':>6s} {'Delta':>7s}")
    # Combine all players from both
...
if dk_hub:
    ...
    if deltas:
        ...
        print(f"  Within 30%: {sum(1 for d in deltas if d <= 30)/len(deltas)*100:.0f}%")

# Key missing players (high DK exposure, 0 ours)
if n_dk > 0:
...
print(f"  Ghost players:       {'ALL ELIMINATED' if all(our_exposure.get(g, 0) == 0 for g in ['Zach Edey', 'Jaren Jackson Jr.', 'Walker Kessler', 'Damian Lillard']) else 'SOME REMAIN'}")
if n_dk > 0:
    print(f"  Top-10 overlap:      {overlap}/10")
    print(f"  Top-20 overlap:      {close_count}/20")
if deltas:
